  ...
  chain_signatures_prod_mpc_rules = [
    {
      ...
      expr          = "(sum by(chain) (rate(multichain_sign_request_latency_sec_count{environment=\"testnet\", step=\"total\", status=\"in_time\"}[$__rate_interval])) / (sum by(chain) (rate(multichain_sign_request_latency_sec_count{environment=\"testnet\", step=\"total\", status=\"in_time\"}[$__rate_interval])) + (sum by(chain) (rate(multichain_sign_request_delayed{environment=\"testnet\"}[$__rate_interval])) or (0 * sum by(chain) (rate(multichain_sign_request_latency_sec_count{environment=\"testnet\", step=\"total\", status=\"in_time\"}[$__rate_interval])))))) * 100"
      threshold     = 95
      comparator    = "lt"
      panel_id      = "149"
