locals {
  ...
  cpu_request_rules = {
    for key, cluster in local.near_clusters : "${key}_cpu_request_high" => {
      ...
      threshold     = 0.90
      comparator    = "gt"
      duration      = key == "dev" ? "3h" : "15m"
      panel_id      = local.cpu_request_panel_ids[key]
      no_data_state = "OK"
      contact_point = cluster.contact_point
      priority      = cluster.priority
      environment   = key
      description   = key == "dev" ? "Workload CPU request utilization has stayed above 90 percent for at least 3 hours. This is sustained under-sizing or persistent load and is worth investigating." : "Workload CPU request utilization has stayed above 90 percent for at least 15 minutes. This usually means requests are too small for current steady-state usage or replicas need tuning."
